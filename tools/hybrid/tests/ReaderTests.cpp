...
static void check(bool value) { if (!value) throw std::runtime_error("Test assertion failed"); }
...
static void u16(std::vector<uint8_t>& b, uint16_t n) { b.push_back(n & 255); b.push_back(n >> 8); }
static void u32(std::vector<uint8_t>& b, uint32_t n) { u16(b, n & 65535); u16(b, n >> 16); }
...
static std::vector<uint8_t> fixture(bool ride)
{
    ...
    if (ride)
    {
        payload[12] = 33; payload[13] = 255; payload[14] = 255;
        name(payload); name(payload);
        payload.insert(payload.end(), 5, 0); // no preset colours or passenger offsets
    }
    const uint32_t count = ride ? 1 : 12;
    u32(payload, count); u32(payload, count * 4);
    for (uint32_t i = 0; i < count; ++i)
    {
        u32(payload, i * 4); u16(payload, 2); u16(payload, 2);
        u16(payload, 0); u16(payload, 0); u16(payload, 1); u16(payload, 0);
    }
    for (uint32_t i = 0; i < count; ++i) payload.insert(payload.end(), { 0, 10, 20, 30 });
...
int main(int argc, char** argv)
{
    try
    {
        ...
        for (bool ride : { false, true })
        {
            ...
            for (size_t n = 0; n < f.size(); ++n) rejects([&] { parse(std::span(f).first(n)); });
            f[17] = 255;
            rejects([&] { parse(f); });
        }
        // Exercise RLE sprite scanlines independently, including out-of-bounds runs.
        std::vector<uint8_t> b;
        u32(b, 1); u32(b, 6); u32(b, 0); u16(b, 2); u16(b, 1);
        u16(b, 0); u16(b, 0); u16(b, 5); u16(b, 0);
        b.insert(b.end(), { 2, 0, 130, 0, 12, 13 });
        Reader r{ b }; check(images(r, 1)[0].pixels == std::vector<uint8_t>({ 12, 13 }));
        b[27] = 1;
        ...
        for (int i = 0; i < 2000; ++i)
        {
            ...
            for (int j = 0; j < 3; ++j) f[rng() % f.size()] = static_cast<uint8_t>(rng());
            try { parse(f); } catch (const std::exception&) {}
        }
        ...
        if (argc > 1)
        {
            ...
            for (const auto& e : std::filesystem::directory_iterator(root / "ObjData"))
            {
                ...
                if (file.size() < 21 || ((file[0] & 15) != 0 && (file[0] & 15) != 8)) continue;
                try { auto d = parse(file); d.type == 0 ? ++rides : ++entrances; }
