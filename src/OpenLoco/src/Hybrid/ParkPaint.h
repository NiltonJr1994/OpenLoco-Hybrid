    ...
    inline void paintPark(Paint::PaintSession& session, const World::Pos2& loc, coord_t height)
    {
        for (const auto& park : Parks::_parks)
        {
            ...
            if (dy == 3 && dx >= -1 && dx <= 1)
            {
                ...
                session.addToPlotListAsParent(ImageId(park.entranceImage + session.getRotation() * 3 + part),
                    { 16, 16, height }, { 2, 2, height }, { 28, 28, 80 });
            }
            for (size_t i = 0; i < park.rides.size(); ++i)
            {
                if (dx != static_cast<int>(i % 3) * 2 - 2 || dy != static_cast<int>(i / 3) * 2 - 2) continue;
                // The real DAT build-menu image is a regional visual proxy,
                // not a simulated or reconstructed track layout.
                session.addToPlotListAsParent(ImageId(park.rides[i].image),
                    { 16, 16, height }, { 2, 2, height }, { 28, 28, 48 });
            }
        }
    }
}
