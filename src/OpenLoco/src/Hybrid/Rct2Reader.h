...
namespace OpenLoco::Hybrid::Rct2
{
    ...
    struct Reader
    {
        ...
        {
            const auto a = u8();
            return static_cast<uint16_t>(a | (u8() << 8));
        }
        uint32_t u32()
        {
            const auto a = u16();
            return a | (static_cast<uint32_t>(u16()) << 16);
        }
        void skip(size_t n)
        {
            require(n);
    ...
    {
        if (encoded.size() > kMaxBytes || encoding > 3)
        {
            throw std::runtime_error("Unsupported RCT2 chunk");
        }
        ...
        while (r.pos < encoded.size())
        {
            ...
            else
            {
                ...
                if (n > kMaxBytes - out.size())
                {
                    throw std::runtime_error("RCT2 expansion limit");
                }
                if (b >= 128)
                {
                    out.insert(out.end(), n, r.u8());
                }
                else
                {
                    r.require(n);
    ...
    inline std::vector<Sprite> images(Reader& r, size_t wanted)
    {
        ...
        if (count == 0 || count > 65536 || size > kMaxBytes)
        {
            throw std::runtime_error("Invalid RCT2 image table");
        }
        ...
        for (size_t i = 0; i < std::min(count, wanted); ++i)
        {
            ...
            Sprite s;
            s.width = static_cast<int16_t>(h.u16());
            s.height = static_cast<int16_t>(h.u16());
            s.x = static_cast<int16_t>(h.u16());
            s.y = static_cast<int16_t>(h.u16());
            ...
            else
            {
                ...
                for (int y = 0; y < s.height; ++y)
                {
                    ...
                    row.pos = row.u16();
                    if (row.pos < static_cast<size_t>(s.height) * 2)
                    {
                        throw std::runtime_error("Invalid RCT2 row offset");
                    }
                    bool ended = false;
                    for (int runs = 0; runs <= s.width; ++runs)
                    {
                        ...
                        if (x + n > static_cast<size_t>(s.width))
                        {
                            throw std::runtime_error("RCT2 sprite row overflow");
                        }
                        row.require(n);
                    ...
                    if (!ended)
                    {
                        throw std::runtime_error("Unterminated RCT2 sprite row");
                    }
                }
            }
    ...
    inline Definition parse(std::span<const uint8_t> file)
    {
        ...
        for (size_t i = 4; i < 12; ++i)
        {
            ...
            {
                throw std::runtime_error("Invalid RCT2 identifier");
            }
            d.id += static_cast<char>(file[i]);
        }
        while (!d.id.empty() && d.id.back() == ' ')
        {
            d.id.pop_back();
        }
        if (d.id.empty())
        {
            throw std::runtime_error("Empty RCT2 identifier");
        }
        r.skip(16);
        const auto encoding = r.u8();
        const auto length = r.u32();
        r.require(length);
        if (length != file.size() - r.pos)
        {
            throw std::runtime_error("RCT2 chunk length mismatch");
        }
        ...
        else if (d.type == 8)
        {
            ...
            if (d.sprites.size() != 12)
            {
                throw std::runtime_error("Entrance requires 12 directional sprites");
            }
        }
        else
        {
            ...
            d.minCars = data.u8();
            d.maxCars = data.u8();
            d.flatRideCars = data.u8();
            data.skip(0x1C2 - data.pos);
            d.name = data.strings();
            d.description = data.strings();
            d.capacity = data.strings();
    ...
    inline Palette readPalette(const std::filesystem::path& path)
    {
        ...
        Reader r{ file };
        const size_t count = r.u32(), size = r.u32();
        if (count <= 1532 || count > 65536)
        {
            throw std::runtime_error("Invalid RCT2 g1 table");
        }
        r.require(count * 16 + size);
        Reader h{ r.bytes, 8 + 1532 * 16 };
        const auto offset = h.u32();
        const auto width = h.u16();
        h.skip(2);
        const auto first = h.u16();
        h.skip(2);
